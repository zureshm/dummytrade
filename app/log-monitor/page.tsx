...
export default function LogMonitorPage() {
  const [sections, setSections] = useState<LogSection[]>([
    { title: "AI Guard", url: `/dummy/api/ai/guard-logs`, logs: [], error: null, filter: "ALL", filterOptions: AI_FILTER_OPTIONS },
  ...
  const clearLogs = async (idx: number) => {
    const section = sectionsRef.current[idx];
    try {
      await fetch(section.url, { method: "DELETE" });
    } catch {}
    setSections((prev) => prev.map((s, i) => (i === idx ? { ...s, logs: [], error: null } : s)));
  };
          ...
          >
            ...
            <div className="px-4 py-2 flex items-center justify-between flex-wrap gap-2" style={{ backgroundColor: "var(--theme-zinc-800)", borderBottom: "1px solid var(--theme-zinc-700)" }}>
              ...
              <div className="flex items-center gap-2">
                ...
                {section.title === "AI Guard" && (
                  <button
                    onClick={() => clearLogs(idx)}
                    className="text-xs px-2 py-0.5 rounded transition-colors hover:opacity-80"
                    style={{
                      background: "var(--theme-zinc-700)",
                      color: "var(--theme-zinc-400)",
                      border: "none",
                      cursor: "pointer",
                    }}
                  >
