...
// GET /api/ai/settings — return current AI guard settings (real apiKey for cross-device visibility)
export async function GET() {
  const settings = getAiGuardSettings();
  return NextResponse.json(settings);
}

// POST /api/ai/settings — save AI guard settings from frontend
export async function POST(request: Request) {
  try {
    ...
    setAiGuardSettings({
      ...
      candlesCount: Number(body.candlesCount) || 90,
